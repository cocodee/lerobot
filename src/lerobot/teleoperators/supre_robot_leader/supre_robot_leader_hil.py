...
from .supre_robot_leader_config import SupreRobotLeaderHilConfig

logger = logging.getLogger(__name__)


class SupreRobotLeaderHil(SupreRobotLeader):
    """
    SO100Follower robot with end-effector space control.

    This robot inherits from SupreRobotLeader but transforms actions from
    end-effector space to joint space before sending them to the motors.
    """

    config_class = SupreRobotLeaderHilConfig
    name = "supre_robot_leader_hil"

    def __init__(self, config: SupreRobotLeaderHilConfig):
