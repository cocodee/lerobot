...
class RobotEnv(gym.Env):
    ...
    def __init__(
        ...
        #DONE: get motors
        self._joint_names = [f"{key}.pos" for key in get_joint_names(self.robot)]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if self.reset_pose is not None:
            ...
            if hasattr(self.env, "robot_leader"):
                #DONE enable
                set_enable_torque(self.env.robot_leader, True)
...
class GripperPenaltyWrapper(gym.RewardWrapper):
    ...
    def reward(self, reward, action):
        ...
        """
        #DONE2 max_grippper_pos
        gripper_state_normalized = self.last_gripper_state / get_max_gripper_position(self.unwrapped.robot)
    ...
    def step(self, action):
        ...
        """
        #DONE: gripper position
        self.last_gripper_state = get_gripper_position(self.unwrapped.robot)
...
class GripperActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        ...
        #DONE
        gripper_command = gripper_command * get_max_gripper_position(self.unwrapped.robot)

        #DONE: get gripper position
        gripper_state = get_gripper_position(self.unwrapped.robot)
        
        #DONE
        gripper_action_value = np.clip(
            gripper_state + gripper_command, 0, get_max_gripper_position(self.unwrapped.robot)
        )
...
class BaseLeaderControlWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        #DONE2: add target_frame_name to robot
        self.kinematics = RobotKinematics(
    ...
    def _handle_intervention(self, action):
        ...
        #DONE: supre robot
        leader_pos_dict = get_present_position(self.robot_leader)
        follower_pos_dict = get_present_position(self.robot_follower)
        ...
        if self.use_gripper:
            if self.prev_leader_gripper is None:
                self.prev_leader_gripper = np.clip(
                    leader_pos[-1], 0, get_max_gripper_position(self.robot_follower)
                )
            ...
            normalized_delta = gripper_delta / get_max_gripper_position(self.robot_follower)
    ...
    def _handle_leader_teleoperation(self):
        ...
        """
        #DONE: supre robot
        prev_leader_pos_dict = get_present_position(self.robot_leader)
        ...
        if not self.leader_torque_enabled:
            set_enable_torque(self.robot_leader,True)
            self.leader_torque_enabled = True

        follower_pos_dict = get_present_position(self.robot_follower)
        follower_pos = np.array([follower_pos_dict[name] for name in follower_pos_dict], dtype=np.float32)

        goal_pos = {f"{motor}": follower_pos[i] for i, motor in enumerate(get_joint_names(self.robot_follower))}
        write_goal_position(self.robot_leader, goal_pos)
    ...
    def step(self, action):
        ...
        #DONE: supre robot
        self.prev_leader_gripper = np.clip(
            get_gripper_position(self.robot_leader),
            0,
...
def record_dataset(env, policy, cfg):
    ...
    while episode_index < cfg.num_episodes:
        ...
        while time.perf_counter() - start_episode_t < cfg.wrapper.control_time_s:
            ...
            # Step environment
            logger.info(f"dkk action {action}")
            obs, reward, terminated, truncated, info = env.step(action)
            logger.info(f"dkk obs {obs}")
